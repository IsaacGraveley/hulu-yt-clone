...
const Thumbnail = forwardRef(({ result }, ref) => {
  ...
  return (
    <div ref={ref} className="p-2 group cursor-pointer transition duration-200 ease-in transform sm:hover:scale-105 hover:z-50">
      <Image
        layout='responsive'
        src={`${BASE_URL}${result.backdrop_path || result.poster.path}` || `${BASE_URL}${result.poster_path}`}
        height={1080}
        width={1920}
      ...
      <div className="p-2">
        ...
        <p className='flex items-center opacity-0 group-hover:opacity-100'>{result.media_type && `${result.media_type} - `}{" "}{result.release_date || result.first_air_date} - {" "}
